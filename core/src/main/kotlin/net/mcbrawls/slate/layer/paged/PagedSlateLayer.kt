...
abstract class PagedSlateLayer(
    ...
    fun createPageChangeTile(title: String, modifier: (Int) -> Int, tile: Tile, callback: TileClickCallback? = null): Tile {
        if (maxPage <= 0) {
            return Tile()
        }

        tile.tooltip(title)
        tile.onGenericClick { slate, t, context ->
            currentPage = modifier.invoke(currentPage)
            updateTileGrid()
            callback?.onClick(slate, t, context)
        }
        return tile
    }

    fun createNextPageTile(tile: Tile, callback: TileClickCallback? = null): Tile {
        return createPageChangeTile("Next Page", { it + 1 }, tile, callback)
    }

    fun createPreviousPageTile(tile: Tile, callback: TileClickCallback? = null): Tile {
        return createPageChangeTile("Previous Page", { it - 1 }, tile, callback)
    }
