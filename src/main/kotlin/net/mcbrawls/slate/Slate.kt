...
open class Slate {
     ...
     */
    open var tiles: HandledTileGrid = TileGrid.create(InventoryType.CHEST_6_ROW)
    ...
    val inventoryType: InventoryType get() = tiles.inventoryType

    private var inventory: SlateInventory<out Slate>? = null
    ...
    internal fun onOpen(player: Player, slateInventory: SlateInventory<out Slate>) {
        // invoke callbacks
    ...
    internal fun onTick(player: Player) {
        if (dirty) {
    ...
    internal fun onClosed(player: Player) {
        if (inventory != null) {
            // clean up
            inventory = null
     ...
     */
    internal fun onAnvilInput(player: Player, input: String) {
     ...
     */
    open fun open(player: Player): Boolean {
        if (inventory != null) {
            if (player != inventory?.openedPlayer) {
                logger.debug("Tried to reopen already opened slate for different player: {}, {}", this, inventory)
            } else {
                logger.debug("Tried to reopen already opened slate: {}, {}", this, inventory)
            }
        ...
        val slateInventory = SlateInventory(this, player, inventoryType, title)
        if (player.openInventory(slateInventory)) {
            inventory = slateInventory
            onOpen(player, slateInventory)
            return true
        }
    ...
    companion object {
        ...
        val Player.slate: Slate? get() = (openInventory as? SlateInventory<*>)?.slate
